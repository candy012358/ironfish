...
function WaitTransactionToBeCompleted() {
    ...
    while [[ ${TRANSACTION_STATUS} != "confirmed" ]] && [[ ${TRANSACTION_STATUS} != "expired" ]]; do
        TRANSACTION_STATUS=$(${BIN} wallet:transaction ${HASH} -d $IRON_PATH | grep "Status: " | sed "s/Status: //")
...
function GetBalanceFunc() {
    if [[ ${1} == '' ]]; then
        ${BIN} wallet:balance -d $IRON_PATH | grep -o "[0-9]\+.[0-9]*" | tail -1
    else
        ${BIN} wallet:balance --assetId=${1} -d $IRON_PATH | grep -Eo "[0-9]+([.][0-9]+)?" | tail -1
    fi
}
...
function MintFunc() {
    ...
    RESULT=$(echo "Y" | ${BIN} wallet:mint --name=${GRAFFITI} --metadata=${GRAFFITI}  --amount=${RANDOM_MINT} --fee=0.00000001 -d $IRON_PATH | tr -d '\0')
...
function BurnFunc() {
    ...
    RESULT=$(echo "Y" | ${BIN} wallet:burn --assetId=${IDENTIFIER} --amount=${BURN_AMOUNT} --fee=0.00000001 -d $IRON_PATH | tr -d '\0')
...
function SendFunc() {
    ...
    RESULT=$(echo "Y" | ${BIN} wallet:send --assetId=${IDENTIFIER} --amount=${SEND_AMOUNT} --to=${ADDRESS_TO_SEND} --memo="${GRAFFITI}" --fee=0.00000001 -d $IRON_PATH | tr -d '\0')
...
function MainFunc() {
    ...
    BIN=$(GetBinaryFunc)
    GRAFFITI=$(echo $(${BIN} config:get blockGraffiti -d $IRON_PATH) | sed 's/\"//g')
